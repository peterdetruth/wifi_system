...
<div class="container-fluid py-3">
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-2 text-end">
            <a href="<?= site_url('admin/dashboard/exportRevenueCSV?month=' . ($month ?? date('m')) . '&year=' . ($year ?? date('Y'))) ?>" class="btn btn-success">Export CSV</a>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        <?php
        $metrics = [
            ['title' => 'Clients', 'value' => $totalClients ?? 0, 'color' => 'primary'],
            ['title' => 'Packages', 'value' => $totalPackages ?? 0, 'color' => 'success'],
            ['title' => 'Revenue (KES)', 'value' => number_format($totalRevenue ?? 0, 2), 'color' => 'warning'],
            ['title' => 'Active Subscriptions', 'value' => $activeSubscriptions ?? 0, 'color' => 'info']
        ];
        ?>
        <?php foreach ($metrics as $m): ?>
            <div class="col-md-3">
                <div class="card text-bg-<?= esc($m['color']) ?> shadow-sm text-center">
                    <div class="card-body">
                        <h6><?= esc($m['title']) ?></h6>
                        <h2 class="count" data-target="<?= esc($m['value']) ?>">0</h2>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>
                            ...
                            <tbody>
                                <?php foreach ($recentTransactions as $t): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge <?= ($t['status'] === 'success') ? 'bg-success' : (($t['status'] === 'failed') ? 'bg-danger' : 'bg-warning text-dark') ?>">
...
function updateCountdowns() {
    document.querySelectorAll('.countdown').forEach(el => {
        ...
        if (diff <= 0) el.innerHTML = '<span class="text-danger fw-bold">Expired</span>';
        else {
